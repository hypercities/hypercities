...
class HCDomDocument {
	...
	protected static $validNamespaces = array(
		'hc' => array (
			'url' => "http://hypercities.ats.ucla.edu/u",
			'count' => 0,
		),
		'atom' => array (
		...
		'gx' => array (
			'url' => "http://www.google.com/kml/ext/2.2",
			'count' => 0,
		)
	);
	...
	public static function getNSURL ($prefix) {
		self::$validNamespaces[$prefix]['count']++;
		return self::$validNamespaces[$prefix]['url'];
	}
...
class HCDomNode {
    ...
    public function createAndAppendNode ($name) {
        if (strrpos($name, ':')) {
			$pieces = explode(':', $name);
            $node = $this->node->appendChild($this->node->ownerDocument->createElementNS(HCKmlDocWrapper::getNSURL($pieces[0]), $name));
        } else {
	...
	/**
	 * Given array of any depth of form elementName => content or array of nodes,
	 * create elements down to given depth.
	 * 
	 * NOTE: not tested yet.
	 * 
	 * @param array $items Array of form key => value; to any depth
	 * @return array of the current node class
	 */
	public function createNodesFromArray(array $items) {
		$return = array();
		foreach ($item as $key => $value) {
			$element = $this->createAndAppendNode($name);
			if (is_array($value)) {
				$element->createNodesFromArray($items);
			} else {
	...
	public function setAttribute ($key, $value) {
		// the following line seems to have bug, strrpos returns false (0) if it does not 
		// find desired string
		//if (strrpos ($key, ':') !== -1) {
		if (strrpos ($key, ':')) {
			// do not set attribute ns
			/*
			$attribute = $this->node->setAttributeNS("http://hypercities.ats.ucla.edu/",
				$key, $value
			);
			 */

...
$.extend(DateEntry.prototype, {
	...
	_extractDate: function(inst) {
		...
		if ( typeof(values[2]) !== 'undefined' && values[2].length > 3 ) {
			year = 1 - year;
		}
		dateObject.setFullYear(year);

		return (month && day ?
			this._daylightSavingAdjust(dateObject) : null);
	},
	...
	_showDate: function(inst) {
		...
		for (var i = 0; i < 3; i++) {
			...
			switch (field) {
				case 'y':
					currentDate += this._formatYear(inst._selectedYear);
					break;
				case 'm':
				...
				case 'd':
					...
					break;
				case 'w': case 'W':
					var dateObject = new Date(inst._selectedYear, inst._selectedMonth, inst._selectedDay);

					dateObject.setFullYear(inst._selectedYear);
					currentDate += this._get(inst, (field == 'W' ? 'dayNames' : 'dayNamesShort'))[
						this._daylightSavingAdjust(dateObject).getDay()] +
						' ' + this._formatNumber(inst._selectedDay);
					break;
			}
		}
		if (inst._selectedYear <= 0) {
			currentDate += " BCE";
		}
		...
		switch (field) {
			...
			case 'w': case 'W':
				...
				return this._get(inst, (field == 'W' ? 'dayNames' : 'dayNamesShort'))
					[this._daylightSavingAdjust(dateObject).getDay()].length + 3;
			default: return 2;
		}
	},
	...
	/* Ensure displayed single number has a leading space.
	   @param  value  (number) current value
	   @return  (string) number with at least four character */
	_formatYear: function(value) {
	...
	_adjustField: function(inst, offset) {
		...
		if ((inst._selectedYear <= 0) && (field == 'y')) {
			offset = 0 - offset;
		}
		...
		var yearOffset = (newYear % 10) - (year % 10);

		dateObject.setFullYear(year + yearOffset);
		this._setDate(inst, this._daylightSavingAdjust(dateObject));
	},
	...
	_getDaysInMonth: function(year, month) {
		var dateObject = new Date(year, month, 32);

		dateObject.setFullYear(year);
		return 32 - dateObject.getDate();
	},
	...
	_determineDate: function(setting) {
		...
		var offsetString = function(offset) { // E.g. '+2m', '-4h', '+3h +30m'
			...
			dateObject.setFullYear(year);
			return $.dateEntry._daylightSavingAdjust(dateObject);
		};
	...
	_handleKeyPress: function(inst, chr) {
		...
		if (chr == dateFormat.charAt(3)) {
			if ( field == 'y' && chr == '-') {
				...
				dateObject.setFullYear(0 - (inst._selectedYear - 1));
				this._setDate(inst, this._daylightSavingAdjust(dateObject));
			} 
			else {
			...
			var year = (field != 'y' ? inst._selectedYear :
					(inst._selectedYear < 1) ? value * -1 + 1: value);
